    ...
    for num in range(len(titles)):
        ...
        for stroke in vklads[num]:
            if stroke[0] == 'Российский рубль':
                ...
                vklad['link'] = 'http://www.banki.ru' + stroke[4]
                if tbf.isDay(del_nt(stroke[3])):
                    vklad['srokinrub'] = get_float(del_nt(stroke[3]))
                elif tbf.isMes(del_nt(stroke[3])):
                    vklad['srokinrub'] = get_float(del_nt(stroke[3])) * 30
                elif tbf.isYear(del_nt(stroke[3])):
                    vklad['srokinrub'] = get_float(del_nt(stroke[3])) * 365
            elif stroke[0] == 'Доллар США':
                vklad['perindollars'] = get_float(stroke[1])
                vklad['sumindollars'] = get_float(stroke[2])
                vklad['srokindollars'] = stroke[3]
            elif stroke[0] == 'Евро':
